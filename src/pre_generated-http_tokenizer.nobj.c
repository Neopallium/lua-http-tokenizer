...
"  \n"
"  local data_len = #data\n"
"  local rc_http_tokenizer_execute = 0\n"
...
"	for n=0,(count-1) do\n"
"		local len = self.tokens[n].len\n"
"		if len > 0 then\n"
"			local start = self.tokens[n].off+1\n"
"			cbs(self.tokens[n].id, data:sub(start, start + len - 1))\n"
"		else\n"
"			cbs(self.tokens[n].id)\n"
"		end\n"
"	end\n"
"\n"
"  return \n"
...
static int http_tokenizer__execute__meth(lua_State *L) {
  ...
  uint32_t rc_http_tokenizer_execute = 0;
  rc_http_tokenizer_execute = http_tokenizer_execute(this, data, data_len);
  lua_pushinteger(L, rc_http_tokenizer_execute);
  return 1;
}

/* method: parse */
static int http_tokenizer__parse__meth(lua_State *L) {
  http_tokenizer * this = obj_type_http_tokenizer_check(L,1);
  size_t data_len;
...
static const luaL_reg obj_http_tokenizer_methods[] = {
  ...
  {"parse", http_tokenizer__parse__meth},
...
LUA_NOBJ_API int luaopen_http_tokenizer(lua_State *L) {
	...
	if(nobj_check_ffi_support(L)) {
		nobj_try_loading_ffi(L, "http_tokenizer", http_tokenizer_ffi_lua_code,
